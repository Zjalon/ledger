# 统计页面重新设计 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the statistics page by replacing four pie chart sections with a unified layout: global time selector, overview cards, trend line chart, and category ranking list.

**Architecture:** Single file `src/pages/stat/Stat.vue` gets rewritten. The script section keeps existing data loading (`refreshData`, `onMounted`, `onUnmounted`) and utility functions (`amountToNumber`, `fmt`, `rangeBounds`), but replaces all computed chart logic. The template replaces four `<section>` blocks with four new ones. The CSS replaces ~810 lines of old styles with new ones.

**Tech Stack:** Vue 3 (`<script setup>` + TypeScript), Vant 4 (`van-tabs`, `van-tab`), SVG (hand-rolled line chart), dayjs
...
|--------|------|----------------|
| Modify | `src/pages/stat/Stat.vue` | Rewrite: new script logic, template, and CSS |

---

### Task 1: Rewrite script section — time selector + overview cards

**Files:**
- Modify: `src/pages/stat/Stat.vue` (lines 1–625)

- [ ] **Step 1: Replace script imports and state variables**

Replace lines 1–36 (everything from `<script setup>` through all the state refs) with:
...
- [ ] **Step 2: Add rangeBounds and periodLabel**

Replace lines 50–81 (old `rangeBounds` + `periodLabel`) with:
...
- [ ] **Step 3: Add overview computed properties**

After `rangeBounds`, add:
...
const incomeLabel = computed(() =>
    fmt.format(amountToNumber(incomeRaw.value)),
);
...
- [ ] **Step 4: Add trend chart computed properties**

After the overview computed properties, add:
...
const trendHasAny = computed(() =>
    trendSeries.value.some((x) => x.amount > 0),
);
...
- [ ] **Step 5: Add category ranking computed properties**

After the trend chart computed properties, add:
...
/** Member tab list for category ranking filter */
...
- [ ] **Step 6: Keep refreshData, onMounted, onUnmounted unchanged**

The existing `refreshData`, `onMounted`, and `onUnmounted` functions (lines 587–624) stay exactly as they are. No changes needed.

- [ ] **Step 7: Verify build**

Run: `pnpm lint`
Expected: PASS (vue-tsc + biome)

- [ ] **Step 8: Commit**

```bash
git add src/pages/stat/Stat.vue
git commit -m "feat(stat): rewrite script with unified time selector, overview, trend, and ranking"
```

---

### Task 2: Rewrite template section

**Files:**
- Modify: `src/pages/stat/Stat.vue` (lines 627–983)

- [ ] **Step 1: Replace entire template**

Replace lines 627–983 (from `<template>` to `</template>`) with:
...
```bash
git add src/pages/stat/Stat.vue
git commit -m "feat(stat): rewrite template with overview, trend, and ranking sections"
```

---

### Task 3: Rewrite CSS section

**Files:**
- Modify: `src/pages/stat/Stat.vue` (lines 985–1793)

- [ ] **Step 1: Replace entire CSS**

Replace lines 985–1793 (from `<style scoped>` to `</style>`) with:
...
```bash
git add src/pages/stat/Stat.vue
git commit -m "feat(stat): rewrite CSS for new layout"
```
...
**Files:**
- Modify: `src/pages/stat/Stat.vue`

- [ ] **Step 1: Remove any dead code**

Check for any unused imports, variables, or functions left over from the old code. The following should have been removed:
- `splitPieRange`, `splitPieFlowKind`, `splitPieCreatorKey`, `memberPieKind` refs
- `piePaths`, `pieTotal`, `expenseShare` computed
- `polar`, `sectorPath`, `fullCircle` functions
- `MEMBER_PIE_COLORS_*` arrays, `memberSliceColor` function
- `hasFlowInRange`, `periodLabel` computed
- `memberMonthPie`, `memberMonthTotalLabel`, `currentMonthBounds`
- `dailyTrendSeries`, `trendMonthTotalLabel`, `trendMonthLabel`
- `splitPieMemberTabList`, `splitPiePeriodLabel`, `splitPieDimHint`, `splitPie`, `splitPieTotalLabel` computed
- `SplitPieSlice`, `MemberMonthSlice` types
...
```bash
git add src/pages/stat/Stat.vue
git commit -m "chore(stat): remove dead code from old layout"
```
...
|------|-------------|---------------|
| 1 | Rewrite script: time selector, overview, trend, ranking computeds | `Stat.vue` |
| 2 | Rewrite template: four new sections | `Stat.vue` |
| 3 | Rewrite CSS: new styles for all sections | `Stat.vue` |
| 4 | Remove dead code, final verify | `Stat.vue` |
